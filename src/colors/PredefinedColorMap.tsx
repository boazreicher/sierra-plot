...
import { ID_PREFIX_TOTAL_ELEMENT, TOTAL_ELEMENT_ID, ID_PREFIX_LINE_ELEMENT, LUMINANCE_DECREASE_SERIES, FILTER_NAME_PREFIX_SELECTED_CHART, FILTER_NAME_PREFIX_GROUP, FILTER_NAME_PREFIX_CHART, FILTER_NAME_PREFIX_SERIES } from "Constants";
...
export class PredefinedColorMap implements ColorPalette {
    ...
    constructor(total: Color, labels: Color, groupPalette: ColorPalette, chartPalette: ColorPalette, seriesPalette: ColorPalette) {
        this.total = total
        this.labels = labels
        ...
        if (elementId.chart !== undefined) {
            if (!this.chartElements.hasOwnProperty(elementId.chart)) {
                ...
                if (selected) {
                    ...
                    this.chartElements[elementId.chart] = new HSLFilter(blend(this.groupElements[group], chartColor).toHsl(), FILTER_NAME_PREFIX_CHART)
                } else {
