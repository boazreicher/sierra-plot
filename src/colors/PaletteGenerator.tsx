...
export class PaletteGenerator {
    ...
    generatePalettes(sortedCharts: ChartData[], panelOptions: SierraPlotOptions): ColorPalettes {
        ...
        colorPalettes.addPalette(PaletteType.Regular, this.generatePalette(panelOptions.labelColor, sortedCharts, panelOptions.color, panelOptions.colorType, panelOptions.colorMode))
        ...
        colorPalettes.addPalette(PaletteType.Selected, this.generatePalette(panelOptions.labelColor, sortedCharts, panelOptions.color, panelOptions.colorType, panelOptions.colorMode, selectedValue))

        return colorPalettes
    }

    private generatePalette(labelColor: string, sortedChartsSeries: ChartData[], colorString: string, colorType: ColorType, colorMode: ColorMode, selectedValue: string | undefined = undefined): ColorPalette {
        var chartPalette: BasicPalette = selectedValue !== undefined ?
        ...
        var palette: PredefinedColorMap = new PredefinedColorMap(this.totalColor, hexToHsl(labelColor), this.groupPalette, chartPalette, this.seriesPalette)

        sortedChartsSeries.forEach(chart => {
            chartPalette.resetIfDifferent(chart.sortKey)
            chart.data.forEach(element => {
                if (element.name !== undefined) {
                    palette.addKey(new ElementId(element.name), selectedValue === undefined ? colorMode : 'regular', selectedValue !== undefined)
                }
            })
        })
