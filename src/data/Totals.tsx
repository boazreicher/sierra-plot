...
import { ID_KV_SEPERATOR, ID_TYPE_CHART_ELEMENT, ID_TYPE_GROUP_ELEMENT, ID_TYPE_SERIES_ELEMENT, TOTAL_ELEMENT_ID } from "Constants"
...
export class Totals {
    ...
    getSelectedTotals(panelOptions: SierraPlotOptions): DataSeries[] {
        if ((panelOptions.totalBreakdown == 'chart' || (panelOptions.totalBreakdown == 'group' && panelOptions.chartsGroupField !== undefined) || (panelOptions.totalBreakdown == 'series' && panelOptions.seriesFieldBreakdown !== undefined))) {
            return this.multi
        } else {
            return [this.single]
        }
    ...
    private calculateTotalsSeries(sortedChartsSeries: ChartData[], panelOptions: SierraPlotOptions): DataSeries[] {
        ...
        if (panelOptions.selectedChart !== undefined && panelOptions.selectedChart.active && panelOptions.selectedChart.type === SelectionType.Group) {
            ...
            switch (panelOptions.totalBreakdown) {
                ...
                case 'chart':
                    ...
                    break
                case 'group':
                    this.getTotalsBreakDownForGroups(sortedChartsSeries, summationsForGroups, panelOptions.totalChartType == 'line' && panelOptions.aggregation == 'avg');
                    break
            }
        }
        ...
        for (var index = 0; index < sortedChartsSeries[0].data[0].dataPoints.length; index++) {
            ...
            for (let group in summationsForGroups) {
                summations100ForGroups[group].dataPoints[index].setY(sum == 0 ? 0 : 100 * summationsForGroups[group].dataPoints[index].y() / sum)
            }
        }
    ...
    private getTotalsBreakDownForSeries(sortedChartsSeries: ChartData[], summationsForGroups: Record<string, DataSeries>, selectedGroup: string | undefined = undefined) {
    ...
    private getTotalsBreakDownForCharts(sortedChartsSeries: ChartData[], summationsForGroups: Record<string, DataSeries>, selectedGroup: string | undefined = undefined) {
        sortedChartsSeries.forEach(chart => {
            if (selectedGroup !== undefined) {
                if (chart.sortKey === undefined) {
                    console.warn("Undefined chart sort key");
    ...
    private getTotalsBreakDownForGroups(sortedChartsSeries: ChartData[], summationsForGroups: Record<string, DataSeries>, average: boolean) {
        sortedChartsSeries.forEach(chart => {
            if (chart.sortKey === undefined) {
