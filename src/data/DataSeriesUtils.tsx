...
import { ID_KV_SEPERATOR, ID_PREFIX_CHART_ELEMENT, ID_SEPERATOR, ID_TYPE_CHART_ELEMENT, ID_TYPE_GROUP_ELEMENT, ID_TYPE_SERIES_ELEMENT, UNKNOWN_SELECTION_VALUE, ZORDER_MISC_BACK } from "Constants"
...
export function getSortedCharts(dataFrames: DataFrame[], properties: ChartsDataProperties): ChartData[] {
    var firstChartName
    if (properties.selection !== undefined && properties.selection.active && properties.selection.type === SelectionType.Chart) {
...
function getCharts(dataFrames: DataFrame[], properties: ChartsDataProperties): Record<string, ChartData> {
    var series: DataSeries[] = extractDataSeries(dataFrames, properties.amplitudeField, properties.weightField, properties.filters, properties.timeRange);

    // This is a side-effect that should be refactored
    properties.formattedTimeRange = normalizeHorizontalAxis(series, properties);

    var charts: Record<string, ChartData> = splitSeriesForCharts(series, properties.breakdowns.chartBreakdown, properties.breakdowns.groupField);

    let breakDownField = properties.breakdowns.seriesBreakdown === undefined || properties.breakdowns.chartBreakdownType == 'none' ?
        properties.breakdowns.chartBreakdown : properties.breakdowns.seriesBreakdown
    charts = sumOverSeriesFieldBreakdown(charts, breakDownField, properties.aggregation);

    return charts;
}

function extractDataSeries(dataFrames: DataFrame[], valueFieldName: string, weightFieldName: string | undefined, filters: SeriesFilters, timeRange: TimeRange): DataSeries[] {
...
function extractDataSeriesValuesAndWeights(valueDataFrames: DataFrame[], weightDataFrames: DataFrame[], filters: SeriesFilters, timeRange: TimeRange): DataSeries[] {
    ...
    for (let index = 0; index < valueDataFrames.length; index++) {
        ...
        for (let index = 0; index < valueField.values.length; index++) {
            ...
            var dataPoint: DataPoint = new DataPoint(new Coordinates(timestamps.get(index), valueField.values.get(index)), weight)
...
function splitSeriesForCharts(series: DataSeries[], splitField: string, groupField: string | undefined): Record<string, ChartData> {
...
export function sortCharts(charts: Record<string, ChartData>, firstChartName: string | undefined, sortMode: SortMode = 'lex'): ChartData[] {
...
export function sumOverSeriesFieldBreakdown(chartsSeries: Record<string, ChartData>, seriesFieldBreakdown: string, aggregation: Aggregation): Record<string, ChartData> {
    let result: Record<string, ChartData> = {}
    ...
    for (let key in chartsSeries) {
        ...
        chartsSeries[key].data.forEach(series => {
            if (!seriesForBreakdownField.hasOwnProperty(series.dimensions[seriesFieldBreakdown])) {
                ...
                seriesForBreakdownField[series.dimensions[seriesFieldBreakdown]].name = generateChartElementName(key, series.dimensions[seriesFieldBreakdown], chartsSeries[key].sortKey)
                seriesForBreakdownField[series.dimensions[seriesFieldBreakdown]].dimensions[seriesFieldBreakdown] = series.dimensions[seriesFieldBreakdown]
            }
        })
