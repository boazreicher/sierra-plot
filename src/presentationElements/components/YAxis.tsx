...
export const YAxis = ({
    ...props
}: React.SVGProps<SVGElement> & { effectiveMaxY: number } & { totalStackMode: StackMode} & { enabled: boolean } & { totalsDimensions?: Dimensions }) => (
...
function buildYAxisProperties(range: ValueRange, totalsDimensions: Dimensions): YAxisProperties {
    return new YAxisProperties(range,
...
function createYAxis(effectiveMaxY: number, totalStackMode: StackMode, enabled: boolean, totalsDimensions?: Dimensions) {
    ...
    const yScale = d3.scaleLinear().domain([properties.range.start, properties.range.end]).range([properties.height, properties.yPos])
    const yAxis = d3.axisLeft(yScale).ticks(4).tickFormat(d3.format(properties.format))

    return <g transform={getTransform(properties)}
        ref={(node) => {
            d3.select(node).style("font-size", "35").call(yAxis as any).selectAll("text").style("text-anchor", "end")
        }}
    />
}
