...
function getXAxisStartX(selection: Selection, startX: number) {
  if (selection !== undefined && selection.active) {
    if (
      ...
      selection.hightlightMode == 'exclusive'
    ) {
      return startX;
    }
    return selection.x;
...
function createXAxis(
  ...
  var timeShift =
    (properties.timeRange.end - properties.timeRange.start) * timeShiftRatio;
  ...
  return (
    <svg
      y={properties.yPos}
      height="100%"
      width={getXAxisElementWidth(properties.startX, properties.width)}
    >
      <g
        ref={(node) => {
          d3.select(node)
