...
export const XAxis = ({
    ...props
}: React.SVGProps<SVGElement> & { sierraPlotProperties: SierraPlotProperties } & { selection: Selection }) => (
    createXAxis(props.sierraPlotProperties, props.selection)
);

function buildXAxisProperties(sierraPlotProperties: SierraPlotProperties, selection: Selection): XAxisProperties {
    return new XAxisProperties(sierraPlotProperties.timeRange,
...
function createXAxis(sierraPlotProperties: SierraPlotProperties, selection: Selection) {
    ...
    const xScale = d3.scaleTime().domain([properties.timeRange.start + timeShift, properties.timeRange.end]).range([properties.startX + properties.minX, properties.startX + properties.width])
    const xAxis = d3.axisBottom(xScale).ticks(30)
    return <svg y={properties.yPos} height="100%" width={getXAxisElementWidth(properties.startX, properties.width)}><g
        ref={(node) => {
            d3.select(node).style("font-size", "35").call(xAxis as any).selectAll("text").style("text-anchor", "end")
                .attr("dx", "-.8em")
                .attr("dy", ".15em")
