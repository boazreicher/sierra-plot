...
import { ID_KV_SEPERATOR, ID_PREFIX_GROUP_ELEMENT, ID_SEPERATOR, ID_TYPE_GROUP_ELEMENT, NO_COLOR, ZORDER_CHART_RANGE, ZORDER_CHART_START } from "Constants";
...
import { getOnClickHandlerForChart, getOnClickHandlerForTotal } from "./EventHandlers";
...
export class ChartElements implements PresentationElement {
    ...
    constructor(charts: ChartData[], totalChart: TotalChartData, panelOptions: SierraPlotOptions, sierraPlotProperties: SierraPlotProperties) {
        this.charts = charts
        this.totalChart = totalChart
        this.panelOptions = panelOptions
        this.sierraPlotProperties = sierraPlotProperties
        this.chartsTooltipData = this.buildTooltipData(charts, sierraPlotProperties.getPalette())
    }

    toSvgElements(elements: SvgElement[]): SvgElement[] {
        let result: SvgElement[] = this.generateSvgElementsForCharts(this.panelOptions, this.sierraPlotProperties)

        if (this.panelOptions.showTotal) {
            result.push(this.generateSvgElementsForTotal(this.panelOptions, this.sierraPlotProperties))
        }

        return result
    }
    
    private buildTooltipData(charts: ChartData[], palette: ColorPalette): Record<string, ToolTipData> {
        ...
        charts.forEach(chart => {
            if (chart.sortKey !== undefined) {
                if (!chartsTooltipData.hasOwnProperty(chart.sortKey)) {
                    ...
                    chartsTooltipData[chart.sortKey].color = palette.getColorFor(new ElementId(ID_PREFIX_GROUP_ELEMENT + ID_SEPERATOR + ID_TYPE_GROUP_ELEMENT + ID_KV_SEPERATOR + chart.sortKey))
                }
    ...
    private generateSvgElementsForTotal(panelOptions: SierraPlotOptions, sierraPlotProperties: SierraPlotProperties): SvgElement {
        let totalElement = this.totalChart.toSvgGroup(panelOptions.selectedChart)
        totalElement.onClick = getOnClickHandlerForTotal(panelOptions, sierraPlotProperties.onOptionsChange)

        return totalElement
    }

    private generateSvgElementsForCharts(panelOptions: SierraPlotOptions, sierraPlotProperties: SierraPlotProperties): SvgElement[] {
        ...
        this.charts.forEach(chart => {
            ...
            chartGroup.zOrder = chartGroup.isHighlighted(panelOptions.selectedChart) ? zOrder - ZORDER_CHART_RANGE : zOrder
            chart.zOrder = chartGroup.zOrder
            zOrder += 2

            chartGroup.onClick = getOnClickHandlerForChart(panelOptions, chart.name, chartGroup.boundingBox.minX, chartGroup.boundingBox.maxY, sierraPlotProperties.onOptionsChange);
