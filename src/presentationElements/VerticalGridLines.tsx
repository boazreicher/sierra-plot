...
import { ID_PREFIX_GRIDLINE_ELEMENT, TOTAL_ELEMENT_ID, ZORDER_BACKGROUND_ELEMENT, ZORDER_CHART_LABEL } from 'Constants';
...
export class VerticalGridLines implements PresentationElement {
  ...
  constructor(enabled: boolean, width: number, color: Color, position: GridLinePosition, ticks: number) {
  ...
  toSvgElements(elements: SvgElement[]): SvgElement[] {
    ...
    let topLeft = new Coordinates(elements[topIndex].boundingBox.minX, elements[topIndex].boundingBox.maxY);
    let topRight = new Coordinates(elements[topIndex].boundingBox.maxX, elements[topIndex].boundingBox.maxY);
    let bottomLeft = new Coordinates(elements[bottomIndex].boundingBox.minX, elements[bottomIndex].boundingBox.maxY);

    let results: SvgElement[] = [];

    let width = topRight.x - topLeft.x;
    for (let index = 0; index < this.ticks + 1; index++) {
      let bottom = new Coordinates(bottomLeft.x + (index * width) / this.ticks, bottomLeft.y);
      let top = new Coordinates(topLeft.x + (index * width) / this.ticks, topLeft.y);
