...
export class SvgGroup extends SvgElement {
    ...
    buildElement(height?: number): JSX.Element {
        ...
        return <Tooltip content={getTooltip(this.tooltipData, this.id)}>
            <g id={this.id} onClick={this.onClick} opacity={this.opacity} onDoubleClick={this.onDoubleClick} onMouseOver={this.onMouseOver} filter={this.filter}>
                {this.children.map(child => <SierraPlotSvgElement element={child} height={maxHeight} />)}
            </g></Tooltip>
    }
...
const getTooltip = (data: ToolTipData, chartName: string | undefined) => (
    <>
        ...
        {Object.keys(data.charts).reverse().map(chart => <p style={getStyle(chart, data.charts[chart], chartName)}>{chart}</p>)}
    </>
);
