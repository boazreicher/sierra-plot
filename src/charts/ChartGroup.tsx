...
import { GROUP_LABEL_CLASS, GROUP_LABEL_MARGIN_X, GROUP_LABEL_MARGIN_Y, ID_KV_SEPERATOR, ID_PREFIX_GROUP_ELEMENT, ID_PREFIX_LINE_ELEMENT, ID_SEPERATOR, ID_TYPE_GROUP_ELEMENT, ZORDER_GROUP_BOUNDS, ZORDER_GROUP_LABEL } from "Constants"
...
export class ChartGroup {
  ...
  toSvgElements(panelOptions: SierraPlotOptions, presentationProperties: ChartGroupPresentationProperties): SvgElement[] {
  ...
  private toPolygon(panelOptions: SierraPlotOptions, presentationProperties: ChartGroupPresentationProperties): SvgElement {
    ...
    if (panelOptions.selectedChart !== undefined && panelOptions.selectedChart.active) {
      ...
      if (panelOptions.selectedChart.type == SelectionType.Group && panelOptions.selectedChart.value == this.name && panelOptions.selectedChart.currentColor !== undefined) {
        element.fill = panelOptions.selectedChart.currentColor
      }
    }

    element.onClick = getOnClickHandlerForGroupBounds(this.name, this.boundingBox, panelOptions, element.fill, presentationProperties.onOptionsChange);
    element.zOrder = ZORDER_GROUP_BOUNDS;

    return element
  }

  private toLabel(presentationProperties: ChartGroupPresentationProperties, textSize: number, selection: Selection | undefined): SvgElement {
