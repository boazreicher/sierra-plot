...
export class BasicChartData extends ChartData {
  ...
  protected getChartLabel(sierraPlotProperties: SierraPlotProperties, panelOptions: SierraPlotOptions): ChartLabel {
    let chartCoordinates = new Coordinates(this.dimensions.startX - CHART_LABEL_MARGIN_X + sierraPlotProperties.minX, this.dimensions.startY - CHART_LABEL_MARGIN_Y)
    return new ChartLabel(this.name, chartCoordinates, panelOptions.chartLabelShiftX, panelOptions.chartLabelSize, panelOptions.labelColor)
  }
  protected getChartsPresentationProperties(sierraPlotProperties: SierraPlotProperties, panelOptions: SierraPlotOptions): ChartsPresentationProperties {
    let palette: ColorPalette = sierraPlotProperties.selectPalette(panelOptions.colorMode, panelOptions.selectedChart, this.sortKey, false, panelOptions.chartBreakdownType)
  ...
  protected getEffectiveStartY(sierraPlotProperties: SierraPlotProperties, numCharts: number): number {
    return sierraPlotProperties.dimensions.height - sierraPlotProperties.chartDimensions.originalHeight * numCharts + sierraPlotProperties.dimensions.startY
  }
